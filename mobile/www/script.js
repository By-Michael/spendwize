...
function renderLogin(initialTab='signin'){
  ...
  function renderForgot(){
    ...
    if(fStep==='send'){
      ...
      btn.addEventListener('click',async()=>{if(btn.disabled)return;fErr='';const emailVal=fEmail.trim();if(!emailVal){fErr='Please enter your email address.';renderForgot();return}if(!emailVal.includes('@')){fErr='Invalid email: missing "@" symbol (e.g. you@gmail.com).';renderForgot();return}const atIdx=emailVal.indexOf('@');const domain=emailVal.slice(atIdx+1);if(!domain||!domain.includes('.')||domain.endsWith('.')||domain.startsWith('.')){fErr='Invalid email: domain must include a "." (e.g. you@gmail.com).';renderForgot();return}const emailRe=/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;if(!emailRe.test(emailVal)){fErr='Please enter a valid email address (e.g. you@gmail.com).';renderForgot();return}btn.disabled=true;btn.textContent='Sending…';try{const otpMeta=await Auth.generateOtp(emailVal);fOtp=otpMeta.message||('We sent a 6-digit code to '+(otpMeta.maskedEmail||emailVal)+'.');fStep='otp';fTimer=60;renderForgot();startForgotTimer()}catch(e){fErr=e.message;btn.disabled=false;btn.textContent='Send OTP Code';renderForgot()}});
      body.append(inp,btn);
    }else if(fStep==='otp'){
      if(fOtp){const _bnr=el('div',{style:{background:'rgba(59,130,246,.15)',border:'1px solid rgba(59,130,246,.3)',borderRadius:'.75rem',padding:'.75rem',fontSize:'.875rem',color:'#bfdbfe',display:'flex',alignItems:'center',gap:'.5rem'}});_bnr.append(ic('mail',14),fOtp);body.appendChild(_bnr)}
      ...
      const vBtn=el('button',{cls:'auth-btn'},'Verify Code');vBtn.addEventListener('click',async()=>{if(vBtn.disabled)return;fErr='';const trimmed=fOtpInput.trim();if(trimmed===''){fErr='Please enter the code.';renderForgot();return}if(trimmed.length!==6){fErr='Please enter the full 6-digit code.';renderForgot();return}vBtn.disabled=true;vBtn.textContent='Verifying…';try{await Auth.verifyOtp(fEmail.trim(),trimmed);fStep='newpw';renderForgot()}catch(e){fErr=e.message;vBtn.disabled=false;vBtn.textContent='Verify Code';renderForgot()}});
      ...
      resBtn.addEventListener('click',async()=>{if(resBtn.disabled)return;resBtn.disabled=true;resBtn.textContent='Sending…';try{const otpMeta=await Auth.generateOtp(fEmail.trim());fOtp=otpMeta.message||('We sent a 6-digit code to '+(otpMeta.maskedEmail||fEmail.trim())+'.');fTimer=60;renderForgot();startForgotTimer()}catch(e){fErr=e.message;resBtn.disabled=false;renderForgot()}});
  ...
  function render(){
    ...
    if(tab==='account'){
      ...
      if(!showReset){const btn=el('button',{cls:'btn-primary'});btn.append(ic('lock',14),' Reset Password via OTP');btn.addEventListener('click',()=>{showReset=true;fStep='send';fOtp='';fOtpInput='';render()});secCard.appendChild(btn)}
        ...
        const _mEmail=fEmail.replace(/^([^@]{1,2})[^@]+(@.*)$/,(_, a, b)=>a+'…'+b);const btn=el('button',{cls:'btn-primary'});btn.append(ic('send',14),' Send Code to '+_mEmail);
        btn.addEventListener('click',async()=>{if(btn.disabled)return;if(!fEmail){fErr='Could not detect your email. Please reload the page.';render();return}fErr='';btn.disabled=true;btn.textContent='Sending…';try{const otpMeta=await Auth.generateOtp(fEmail);fOtp=otpMeta.message||('We sent a 6-digit code to '+(otpMeta.maskedEmail||fEmail)+'.');fStep='verify';fTimer=60;render();}catch(e){fErr=e.message;btn.disabled=false;btn.textContent='Send OTP to Email';render()}});secCard.appendChild(btn);
      }else if(fStep==='verify'){
        if(fOtp){const _bnr=el('div',{style:{background:'rgba(59,130,246,.15)',border:'1px solid rgba(59,130,246,.3)',borderRadius:'.75rem',padding:'1rem',fontSize:'.875rem',color:'#bfdbfe',display:'flex',alignItems:'center',gap:'.5rem'}});_bnr.append(ic('mail',14),fOtp);secCard.appendChild(_bnr)}
        ...
        const vBtn=el('button',{cls:'btn-primary flex-1',style:{justifyContent:'center'}},'Verify Code');vBtn.addEventListener('click',async()=>{if(vBtn.disabled)return;fErr='';const trimmed=fOtpInput.trim();if(trimmed===''){fErr='Please enter the code.';render();return}if(trimmed.length!==6){fErr='Please enter the full 6-digit code.';render();return}vBtn.disabled=true;vBtn.textContent='Verifying…';try{await Auth.verifyOtp(fEmail,trimmed);fStep='newpw';render()}catch(e){fErr=e.message;vBtn.disabled=false;vBtn.textContent='Verify Code';render()}});
        const rBtn=el('button',{cls:'btn-secondary flex-1',style:{justifyContent:'center'}},fTimer>0?fTimer+'s':'Resend');securityTimerButtons.push(rBtn);syncSecurityTimerButtons();rBtn.addEventListener('click',async()=>{if(rBtn.disabled)return;rBtn.disabled=true;rBtn.textContent='Sending…';try{const otpMeta=await Auth.generateOtp(fEmail);fOtp=otpMeta.message||('We sent a 6-digit code to '+(otpMeta.maskedEmail||fEmail)+'.');fTimer=60;render();}catch(e){fErr=e.message;rBtn.disabled=false;render()}});
        row.append(vBtn,rBtn);secCard.append(oIn,row);
        // Restart the interval after each render() so it always holds the freshly-created rBtn.
        if(fTimer>0)startSecurityTimer();
      }else if(fStep==='newpw'){
